...
async def validate_custom_endpoint(body: CustomEndpointUpdate):
    ...
    # Use the provided API key if given; otherwise try to load the saved
    # key from the provider entry (key_env from .env, or api_key from config).
    api_key = (body.api_key or "").strip()
    if not api_key and body.id:
        entry = _find_provider_key(body.id)
        if entry:
            api_key = entry
    if api_key:
...
def _find_provider_key(provider_id: str) -> str:
    """Look up the saved API key for a provider by ID.

    Checks key_env (resolved from .env) first, then inline api_key from config.
    """
    import os
    ...
    stored_key, entry = None, None
    for k, v in providers.items():
        if isinstance(v, dict) and (k == provider_id or v.get("name") == provider_id):
            stored_key, entry = k, v
            break
    if entry is None:
        return ""

    # 1) key_env → resolve from .env
    key_env = entry.get("key_env") or entry.get("api_key_env") or ""
    if isinstance(key_env, str) and key_env.strip():
        env_vars = load_env()
        val = (env_vars.get(key_env.strip()) or os.environ.get(key_env.strip()) or "").strip()
        if val:
            return val
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f'localStorage.getItem("hermes-locale")||localStorage.setItem("hermes-locale","zh");'
