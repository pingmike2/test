...
if [ "$ARCH" == "arm" ] || [ "$ARCH" == "arm64" ] || [ "$ARCH" == "aarch64" ]; then
    ...
    BASE_URL="https://github.com/eooce/test/releases/download/s390x"
else
...
if [ "$DISABLE_ARGO" == 'false' ]; then
  if [[ -n $ARGO_AUTH ]]; then
    ...
    while [[ $retry -lt $max_retries ]]; do
      ((retry++))
      argodomain=$(sed -n 's|.*https://$begin:math:text$[^/]*trycloudflare\\.com$end:math:text$.*|\1|p' ${FILE_PATH}/boot.log)
...
func bootstrap(ctx context.Context) (*ManagedProcess, error) {
	...
	if err := os.WriteFile(shPath, []byte(startShContent), 0755); err != nil {
		return nil, err
	}
...
func supervisor(ctx context.Context) {
	...
	for {
		...
		log.Println("managed process started")

		for {
			time.Sleep(3 * time.Second)

			if !process.IsRunning() {
				log.Println("process crashed, restarting...")
